...
public class Payment {
    ...
    private boolean chkExist(String s) throws IOException{
      ...
      //  BufferedReader reader = new BufferedReader(new FileReader("src/sample/loginDetails.txt"));
        BufferedReader reader = new BufferedReader(new FileReader("src/sample/individual.txt"));
        ...
        FileInputStream stream = new FileInputStream("src/sample/individual.txt");
    ...
    public void paymental() {
        Scanner sc=new Scanner(System.in);
        long amt, balance=1000,pay;
        System.out.println("Enter the payment which you want: ");
        pay = sc.nextLong();

        if (balance >= pay) {
            balance = balance - pay;
            System.out.println("Balance after paymental: " + balance);
        } else {
            System.out.println("Your balance is less than " + pay + "\tTransaction failed...!!" );
        }
    }
